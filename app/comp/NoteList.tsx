...
export default function Notetiles() {
    return (
        <div className="mx-8 mb-7 mt-16 grid justify-center text-zinc-200">
            {NoteList.map(note => (
                <div className="md:w-[704px] xl:w-[820px] lg:w-[820px]">
                    <Link className="my-1.5 flex justify-between antialiased text-wrap hover:scale-[1.01] duration-100 bg-neutral-900 px-5 py-3.5 rounded-lg" to={'/' + note.noteid}>
                        <div className=" inline-flex">
                            <p className="text-lg text-wrap line-clamp-1">{note.title}</p>
                        </div>
                        <div className="inline-flex min-w-fit items-center">
                            <p className="min-w-min px-4 inline-flex">{note.creationDate?.dd}-{note.creationDate?.mm}-{note.creationDate?.yyyy}</p>
                            <form className="inline-flex items-center" action="/" method="get">
                                <input type="hidden" value={note.noteid} />
                                <button type="submit" className=" p-1 rounded bg-slate-500 ">
